...
contract MCOContract is Ownable {
    ...
    // Tracks the total income for each beneficiary
    mapping(address => uint256) private _beneficiaryIncome;
    ...
    function updateIncomePercentage(address beneficiary, uint256 percentage) public onlyOwner {
        require(_parties.contains(beneficiary), "Invalid beneficiary address");

        uint256 totalPercentage = 0;
        EnumerableMap.AddressToUintMap storage incomeMap = _incomePercentages[beneficiary];
        uint256 l = incomeMap.length();

        for (uint256 i = 0; i < l; i++) {
            (, uint256 incomePercentage) = incomeMap.at(i);
            totalPercentage += incomePercentage;
        }

        require(totalPercentage + percentage <= 100, "Total income percentage exceeds 100");
        incomeMap.set(beneficiary, percentage);
    }
